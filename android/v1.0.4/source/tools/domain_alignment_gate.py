...
if PATCH_B64.exists() and not (ROOT / 'app/src/main/java/dev/securemesh/commander/data/ble/BleDiscoveryParityTransport.kt').exists():
    try:
...
check("Firmware v1.0.4 operational opcodes appended without renumbering", all(x in CODEC for x in ["GET_BLE_RADAR(28)", "CLEAR_BLE_RADAR(29)", "GET_OPERATIONAL_HEALTH(30)", "GET_SELF_DIAGNOSTICS(31)"]))
check("Firmware v1.0.4 health event appended", "OPERATIONAL_HEALTH_CHANGED(32)" in CODEC)
check("No historical conflicting 0.9.3 opcode assignments", "FIND_DEVICE(28)" not in CODEC and "RUN_SELF_TEST(29)" not in CODEC)
check("Full v0.9.2 feature surface retained", all((MAIN / p).exists() for p in ["core/map/OfflineMapManager.kt", "core/security/SensitiveDataCipher.kt", "data/ble/BleDiscoveryParityTransport.kt", "feature/security/SecurityCenterScreen.kt", "feature/vanguard/VanguardControlScreen.kt"]))
...
check("MapLibre local PMTiles renderer is integrated", "org.maplibre.gl:android-sdk" in BUILD_TEXT and "MapLibreMeshMapProvider" in MAPLIBRE and "localVectorStyle" in MAPLIBRE and "pmtiles" in (MAIN / "core/map/OfflineMapManager.kt").read_text(encoding="utf-8"))
...
MAP_MANAGER = (MAIN / "core/map/OfflineMapManager.kt").read_text(encoding="utf-8")
check("Internet is limited to explicit offline-map download surface", "android.permission.INTERNET" in MANIFEST and 'uri.scheme != "https"' in MAP_MANAGER and "usesCleartextTraffic=\"false\"" in MANIFEST)
check("Map packs are validated before atomic install", all(x in MAP_MANAGER for x in ["PmTilesHeaderParser.read(source)", "PmTilesHeaderParser.read(staging)", ".installing", "renameTo(destination)"]))
check("Map manager supports download import select delete", all(x in MAP_MANAGER for x in ["startDownload", "importFromUri", "fun select", "fun delete", "DownloadManager"]))
check("Normal map UI hides engineering GPS jargon", all(x not in MAP_SCREEN for x in ["Command Map", "FIX $", "LAST $", "HDOP", "sats ", "Follow", "DATA / SEARCH / FIX", "NEO-7M → ESP32"]))
check("Position history drives map tracks", "positionHistory" in MAP_SCREEN and "tracks = tracks" in MAP_SCREEN)

VAULT = (MAIN / "core/security/SensitiveDataCipher.kt").read_text(encoding="utf-8")
CONTACT_EDITOR = (MAIN / "feature/nodes/ContactEditorDialog.kt").read_text(encoding="utf-8")
check("Android Keystore AES-256-GCM vault present", all(x in VAULT for x in ["AndroidKeyStore", "AES/GCM/NoPadding", ".setKeySize(256)", ".setRandomizedEncryptionRequired(true)", "updateAAD"]))
check("Contact aliases and notes are encrypted", all(x in REPO for x in ["contact:$nodeId:alias", "contact:$nodeId:note", "encryptedAlias", "encryptedNote"]))
check("Message bodies are encrypted at rest", 'cipher.encryptString(payload, "message:${stableKey()}")' in REPO and "hardenLegacyMessagePayloads" in REPO)
check("GPS history is encrypted at rest", "position:$key" in REPO and "encryptedPayload" in ENTITIES and "DROP TABLE IF EXISTS `position_history`" in DATABASE)
check("Contact editor preserves trust identity", "Node ID $nodeId остаётся неизменной trust-identity" in CONTACT_EDITOR and "repository.updateContactProfile" in (MAIN / "feature/nodes/NodeDetailsViewModel.kt").read_text(encoding="utf-8"))
check("SQLite secure delete is enabled", "PRAGMA secure_delete=ON" in DATABASE)
...
check("Canonical Android v1.0.4 version is stamped", 'versionName = "1.0.4"' in (ROOT / "app/build.gradle.kts").read_text())
...
check("Room trust/message/contact/vault migrations exist", all(x in DATABASE for x in ["Migration(1, 2)", "Migration(2, 3)", "Migration(3, 4)", "Migration(4, 5)", "MIGRATION_4_5"]))
...
check("No unsafe high-arity combine in NodesViewModel", "private val controls = combine(query, filters, sort)" in NODES_VM and "repository.contactProfiles, controls" in NODES_VM)
...
RADAR_SCREEN = (MAIN / "feature/radar/BleRadarScreen.kt").read_text(encoding="utf-8")
RADAR_VM = (MAIN / "feature/radar/BleRadarViewModel.kt").read_text(encoding="utf-8")
LINK_HEALTH = (MAIN / "domain/service/LinkHealthV104.kt").read_text(encoding="utf-8")
MESH_TRANSPORT = (MAIN / "data/transport/MeshTransport.kt").read_text(encoding="utf-8")
REPO_INTERFACE = (MAIN / "domain/repository/SecureMeshRepository.kt").read_text(encoding="utf-8")
check("Node BLE Radar is repository-backed, not a phone scanner", "repository.bleDiagnostics" in RADAR_VM and "BluetoothLeScanner" not in RADAR_SCREEN + RADAR_VM and "ScanCallback" not in RADAR_SCREEN + RADAR_VM)
check("CLEAR_BLE_RADAR is wired end-to-end", all(x in (MESH_TRANSPORT + REPO_INTERFACE + REPO + BLE + CODEC) for x in ["clearBleRadar", "CLEAR_BLE_RADAR(29)", "parseClearBleRadar"]))
check("Link Health v1.0.4 is display-only", "does not replace firmware" in LINK_HEALTH.lower() and "never sent back" in LINK_HEALTH.lower() and "BluetoothGatt" not in LINK_HEALTH)
check("Cross-contract gate is present", (ROOT / "tools/ble_contract_gate.py").exists())

print("SecureMesh Android v1.0.4 canonical alignment gate")
