...
interface SecureMeshDao {
    ...
    @Insert(onConflict = OnConflictStrategy.REPLACE)
    suspend fun upsertContactProfile(item: ContactProfileEntity)
    @Query("SELECT * FROM contact_profiles ORDER BY updatedAtEpochMs DESC")
    fun observeContactProfiles(): Flow<List<ContactProfileEntity>>
    @Query("DELETE FROM contact_profiles WHERE nodeId = :nodeId")
    suspend fun deleteContactProfile(nodeId: String)
