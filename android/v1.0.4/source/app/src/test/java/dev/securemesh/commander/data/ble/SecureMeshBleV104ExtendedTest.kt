...
class SecureMeshBleV104ExtendedTest {
    ...
    @Test fun `position radar and sos payloads decode with current firmware layout`() {
        ...
        assertEquals("AABBCCDD", decoded.sos.origin)
        assertEquals(77, decoded.sos.batteryPercent)
    }

    @Test fun `v1_0_4 capability and route source mapping is current`() {
        val caps = SecureMeshBleV02DomainMapping.capabilities(
        ...
        assertEquals(RouteType.DYNAMIC, SecureMeshBleV02DomainMapping.route(BleRoutePayload("00000002","00000003",2), 1)?.type)
        assertEquals(RouteType.STATIC, SecureMeshBleV02DomainMapping.route(BleRoutePayload("00000002","00000003",4), 1)?.type)
    }

    @Test fun `v1_0_4 extends but never renumbers the 0_9_2 command surface`() {
        fun opcode(command: SecureMeshBleCommand) = codec.encodeCommand(7, command).getOrThrow()[6].toInt() and 0xFF
        assertEquals(13, opcode(SecureMeshBleCommand.GetUiState))
        assertEquals(15, opcode(SecureMeshBleCommand.GetKnownNodes))
        assertEquals(19, opcode(SecureMeshBleCommand.GetRoutingDiagnostics))
        assertEquals(22, opcode(SecureMeshBleCommand.SetLabLinkPolicy("00000002", 2, 30_000)))
        assertEquals(24, opcode(SecureMeshBleCommand.GetPositions))
        assertEquals(27, opcode(SecureMeshBleCommand.SendCommandNotice("00000002", 1)))
        assertEquals(28, opcode(SecureMeshBleCommand.GetBleRadar))
        assertEquals(29, opcode(SecureMeshBleCommand.ClearBleRadar))
        assertEquals(30, opcode(SecureMeshBleCommand.GetOperationalHealth))
        assertEquals(31, opcode(SecureMeshBleCommand.GetSelfDiagnostics))
        assertEquals(32, BleEventType.OPERATIONAL_HEALTH_CHANGED.wire)
    }
    ...
    @Test fun clearBleRadarAcceptsOnlyEmptyOkPayload() {
        val codec = SecureMeshBleProtocolV02Codec()
        val ok = SecureMeshBleFrame.Response(1, BleOpcode.CLEAR_BLE_RADAR, 29, BleCommandStatus.OK, 0, byteArrayOf())
        assertTrue(codec.parseClearBleRadar(ok).isSuccess)
        val wrong = SecureMeshBleFrame.Response(1, BleOpcode.CLEAR_BLE_RADAR, 29, BleCommandStatus.OK, 0, byteArrayOf(1))
        assertTrue(codec.parseClearBleRadar(wrong).isFailure)
    }
}
