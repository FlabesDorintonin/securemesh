...
class RootViewModel(private val repository: SecureMeshRepository) : ViewModel() {
    val sos = repository.activeSos
    val session = repository.session
    val connection = repository.connectionState
    val transportMode = repository.transportMode

    fun acknowledge(id: String) = viewModelScope.launch { repository.acknowledgeSos(id) }
}
