...
fun SecurityCenterScreen(viewModel: SecurityCenterViewModel) {
    ...
    MeshBackdrop(Modifier.fillMaxSize()) {
        LazyColumn(
            ...
            item {
                SecurityCard(Icons.Rounded.PhoneAndroid, "Локальные данные", "Только устройство", true) {
                    Text("У приложения нет INTERNET permission. Android backup и device-transfer backup отключены; рабочая Room/DataStore база остаётся во внутреннем хранилище приложения.", color = SecureMeshColors.TextSecondary, style = MaterialTheme.typography.bodySmall)
                    Text("Экспортированный вручную CSV/JSON уже находится вне защищённого хранилища приложения — обращайся с ним как с обычным файлом.", color = SecureMeshColors.Warning, style = MaterialTheme.typography.bodySmall)
                }
            }
