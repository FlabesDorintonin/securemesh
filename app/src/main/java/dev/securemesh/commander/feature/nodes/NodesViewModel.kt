...
data class NodeListItem(val node: MeshNode, val primaryLink: MeshLink?)
...
class NodesViewModel(repository: SecureMeshRepository) : ViewModel() {
    ...
    val uiState = combine(repository.nodes, repository.topology, repository.session, controls) { nodes, topology, session, controls ->
        ...
        var sequence = visibleNodes.asSequence()
            .map { NodeListItem(it, primaryLink(it.id)) }
            .filter { text.isBlank() || it.node.id.lowercase().contains(text) || it.node.name.lowercase().contains(text) }
        ...
        val list = when (controls.sort) {
            NodeSort.NAME -> sequence.sortedBy { it.node.name }
