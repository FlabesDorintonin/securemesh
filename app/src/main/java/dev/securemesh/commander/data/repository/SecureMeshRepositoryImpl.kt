        ...
        scope.launch {
            session.filterNotNull().collect { secureSession ->
                ...
                if (previousHistoryOwner != null && previousHistoryOwner != identity.nodeId) {
                    // Events/positions/tests are single-session operational history. Messages are different: each row already
                    // contains origin/destination, so they can remain safely partitioned by local SecureMesh node identity.
