...
object SecureMeshBleFragmentation {
    ...
    fun fragment(packet: ByteArray, mtu: Int, transportId: Int): Result<List<ByteArray>> = runCatching {
        require(packet.size in SecureMeshBleProtocolV01Codec.HEADER_SIZE..MAX_APPLICATION_PACKET) { "application packet must be 10..384 bytes" }
    ...
    class Reassembler(private val timeoutMs: Long = REASSEMBLY_TIMEOUT_MS) {
        ...
        fun accept(fragment: ByteArray, nowMs: Long): AcceptResult {
            ...
            if (decoded.totalLength !in SecureMeshBleProtocolV01Codec.HEADER_SIZE..MAX_APPLICATION_PACKET) return reject("invalid totalLength")
